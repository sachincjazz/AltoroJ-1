node{
  ...
  stage ('cloning the repository'){
      git 'https://github.com/sachincjazz/AltoroJ-1.git'
  }
  ...
  stage('Gradle Build') {
       //def gradleHome = tool name : 'mygradle', type 'gradle',
        ...
        sh "C:/gradle-4.7/bin/gradle build"
   }
  ...
  stage('Publish Artificats to UCD'){
   step([$class: 'UCDeployPublisher',
        ...
        component: [
		 ...
		 baseDir: 'C:/HCL/Jenkins/workspace/AltoroJ/build/libs/',
	...
	echo "(*****)"
	  //echo "Demo1234 ${AltoroComponent_VersionId}"
	   newComponentVersionId = "${AltoroComponent_VersionId}"
